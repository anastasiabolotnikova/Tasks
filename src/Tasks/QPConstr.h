...
public:
  ...
  void addGripper(const ContactId & cId,
                  double torqueLimit,
...
private:
  struct GripperData
  {
    GripperData(const ContactId & cId, double tl, const Eigen::Vector3d & o, const Eigen::Vector3d & a);
...
private:
  ...
  {
    BoundedSpeedData(rbd::Jacobian j,
                     const Eigen::MatrixXd & d,
                     const Eigen::VectorXd & ls,
                     const Eigen::VectorXd & us,
                     const std::string & bName)
    : jac(j), bodyPoint(j.point()), dof(d), lSpeed(ls), uSpeed(us), body(j.jointsPath().back()), bodyName(bName)
    {
    }
...
public:
   ...
   */
  ImageConstr(const std::vector<rbd::MultiBody> & mbs,
              int robotIndex,
              const std::string & bName,
   ...
   * @param damping
   */
  void setLimits(const Eigen::Vector2d & min,
   ...
   * @param X_b_p
   */
  void addPoint(const std::vector<rbd::MultiBody> & mbs,
                const std::string & bName,
  ...
  void computeComponents(const rbd::MultiBody & mb,
